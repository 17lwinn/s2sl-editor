...
document.getElementById(`${package.name}Save`).onclick = function() {
  os.prompt("Please type in the path of where you want to save.", "Save", window.id, function(path) { os.filesystem.writeFile(path, document.getElementById(`${package.name}Text`).value); });
}
})()